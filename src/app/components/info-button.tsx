import { ArrowUp, Info, X } from '@phosphor-icons/react'
...
const InfoModal = ({ note, onClose }: { note: RegulationNote; onClose: () => void }) => {
  ...
  return (
    ...
    >
      <section
        className="relative flex max-h-[min(86vh,720px)] w-full max-w-xl flex-col overflow-hidden rounded-[1.6rem] border border-sky-200/90 bg-sky-50/95 shadow-[0_24px_64px_-24px_rgba(15,23,42,0.32)]"
        onClick={(event) => event.stopPropagation()}
      >
        <header className="flex items-start justify-between gap-3 border-b border-sky-200/60 bg-sky-50/95 px-6 pb-4 pt-5">
        ...
        <div
          ...
          ref={bodyRef}
        >
          <div className="rounded-[1.1rem] border border-sky-100/80 bg-white/80 p-4 whitespace-pre-line text-sm leading-relaxed text-zinc-700">
            {note.body}
          </div>
          {note.source ? (
        ...
        {showScrollTop ? (
          <button
            aria-label="Наверх"
            className="absolute bottom-4 right-4 inline-flex h-10 w-10 items-center justify-center rounded-full border border-sky-200 bg-white text-sky-600 shadow-[0_10px_24px_-10px_rgba(14,165,233,0.45)] transition hover:border-sky-400 hover:text-sky-700"
            data-testid="info-modal-scroll-top"
            onClick={scrollToTop}
            type="button"
          >
