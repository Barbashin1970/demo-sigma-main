...
> Живая версия: <https://sigma-operator.vercel.app/operator/hospital-fire>
>
...
| :-- | :-- |
| `/operator/:scenarioId` | Рабочая станция оператора. Транспорт (Шаг/Сброс), AI-подсказки Sigma Assist, каталог сценариев, быстрые переходы в тренажёр и регламенты. |
| `/display/:scenarioId` | Видеостена. Read-only, управление — через BroadcastChannel из оператор-вкладки. |
| `/trainer/:scenarioId` | Тренажёр. Оператор выбирает действие из `allowedActions`, получает балл, в конце — анкета стажёра и аттестационный отчёт JSON. В шапке — i-кнопка с инструкцией. |
| `/regulations` | Просмотрщик регламентов: витрина всех `situation` / `actions` по сценариям + «Чего не делать» по 5 RiskKind. Скачать YAML/JSON. Открывается из Sigma Assist. |
